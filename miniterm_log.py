...
import sys, os, serial, threading, logging
try:
    ...
    def reader(self):
        ...
        try:
            ...
            while self.alive and self._reader_alive:
                ...
                # log
                if data in '\r\n':
                    if line != "": # ignore empty lines (and thus log CR/LF/CRLF as system defined newline)
                        logging.info(line)
                        line = ""
                else:
                    line += data
...
3: hex dump everything""",
    ...
    if options.logfile is not None:
        logging.basicConfig(filename=options.logfile, level=logging.DEBUG, format='%(asctime)s %(message)s')
