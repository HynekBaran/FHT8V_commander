...
sub PID20_Calc($)
{
   ...
   # update statePID.
    $stateStr = sprintf("idle: act = $actuation, delta = %.1f C, (p=%.1f, i=%.1f, d=%.1f)", $delta, $pPortion, $iPortion, $dPortion)  if (!$stateStr && !$calcReq); # HB
    $stateStr = sprintf("processing: act = $actuation, delta = %.1f C, (p=%.1f, i=%.1f, d=%.1f)", $delta, $pPortion, $iPortion, $dPortion)  if (!$stateStr && $calcReq); # HB
